...
const { client, server } = createVoltcode()
...
try {
  ...
  console.log("voltcode session", session.id)
  if (shareId) {
...
function createVoltcode() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`voltcode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createVoltcodeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/voltcode|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/voltcode` or `/oc`")
  }
}
...
async function assertVoltcodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to voltcode server")
  }
}
...
async function getAccessToken() {
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("voltcode-github-action")
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/voltcode" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/voltcode") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to voltcode...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "voltcode-agent[bot]"`
  await $`git config --global user.email "voltcode-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `voltcode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const shareUrl = shareId ? `[voltcode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
