...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // voltcode bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-voltcode-session") ?? ""
    const requestId = input.request.headers.get("x-voltcode-request") ?? ""
    const projectId = input.request.headers.get("x-voltcode-project") ?? ""
    const ocClient = input.request.headers.get("x-voltcode-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-voltcode-request")
          headers.delete("x-voltcode-session")
          headers.delete("x-voltcode-project")
          headers.delete("x-voltcode-client")
          return headers
        })(),
        body: reqBody,
      })
