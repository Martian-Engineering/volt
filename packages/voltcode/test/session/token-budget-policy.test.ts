...
describe("TokenBudget.computeDoltLanePolicy", () => {
  test("uses pagedrop defaults for bindles and leaves", () => {
    const policy = TokenBudget.computeDoltLanePolicy({ hardLimit: 120000, softThreshold: 70000 })
    ...
    expect(policy.turns.cap).toBe(120000)
    expect(policy.turns.soft).toBe(70000)
    expect(policy.turns.delta).toBe(0)
    expect(policy.turns.target).toBe(70000)
  ...
  test("supports env overrides and clamps turns to cap", () => {
    ...
    const policy = TokenBudget.computeDoltLanePolicy({ hardLimit: 90000, softThreshold: 60000 })
