...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "voltcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .voltcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const voltcodeDir = path.join(dir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })
      const agentDir = path.join(voltcodeDir, "agent")
...
test("loads agents from .voltcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const voltcodeDir = path.join(dir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })

      const agentsDir = path.join(voltcodeDir, "agents")
...
test("loads commands from .voltcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const voltcodeDir = path.join(dir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })

      const commandDir = path.join(voltcodeDir, "command")
...
test("loads commands from .voltcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const voltcodeDir = path.join(dir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })

      const commandsDir = path.join(voltcodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "voltcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .voltcode config
      const projectDir = path.join(dir, "project")
      const voltcodeDir = path.join(projectDir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })
      ...
      // Local .voltcode config with different plugins
      await Bun.write(
        path.join(voltcodeDir, "voltcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .voltcode config with some overlapping plugins
      await Bun.write(
        path.join(voltcodeDir, "voltcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("local .voltcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .voltcode directory config enables it
      const voltcodeDir = path.join(dir, ".voltcode")
      await fs.mkdir(voltcodeDir, { recursive: true })
      await Bun.write(
        path.join(voltcodeDir, "voltcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/voltcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/voltcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-voltcode@2.4.3")).toBe("oh-my-voltcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@voltcode/plugin@2.0.0")).toBe("@voltcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-voltcode@2.4.3", "file:///project/.voltcode/plugin/oh-my-voltcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.voltcode/plugin/oh-my-voltcode.js")
  })
  ...
  test("local plugin directory overrides global voltcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const voltcodeDir = path.join(projectDir, ".voltcode")
        const pluginDir = path.join(voltcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
