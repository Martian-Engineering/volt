...
function usageAndExit(): never {
  console.error(
    "Usage: bun packages/voltcode/scripts/parse_trec.ts --input <context.txt> --output <batches.jsonl> [--batch-size <n>]",
  )
  process.exit(1)
}
...
console.log(`Parsed ${items.length} items from ${inputPath}`)
