...
export namespace LLM {
  ...
  export async function stream(input: StreamInput) {
    ...
    // When precomputedSystem is provided (from TokenBudget), use it directly
    // and only append user.system. Skip inline assembly and plugin transform
    // since those were already applied during caching.
    let system: string[]
    if (input.precomputedSystem) {
      system = [...input.precomputedSystem, ...(input.user.system ? [input.user.system] : [])]
    } else {
      system = SystemPrompt.header(input.model.providerID)
      system.push(
        [
    ...
    return streamText({
      ...
      headers: {
        ...(isCodex
          ? {
              originator: "voltcode",
              "User-Agent": `voltcode/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
              session_id: input.sessionID,
            }
          : undefined),
        ...(input.model.providerID.startsWith("voltcode")
          ? {
              "x-voltcode-project": Instance.project.id,
              "x-voltcode-session": input.sessionID,
              "x-voltcode-request": input.user.id,
              "x-voltcode-client": Flag.VOLTCODE_CLIENT,
            }
          : input.model.providerID !== "anthropic"
            ? {
                "User-Agent": `voltcode/${Installation.VERSION}`,
              }
            : undefined),
        ...input.model.headers,
      },
