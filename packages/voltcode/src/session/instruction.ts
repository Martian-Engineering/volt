...
function globalFiles() {
  ...
  if (Flag.VOLTCODE_CONFIG_DIR) {
    files.push(path.join(Flag.VOLTCODE_CONFIG_DIR, "AGENTS.md"))
  }
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.VOLTCODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no VOLTCODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.VOLTCODE_CONFIG_DIR, Flag.VOLTCODE_CONFIG_DIR).catch(() => [])
}
