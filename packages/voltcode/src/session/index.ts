...
export namespace Session {
  ...
  export const Info = z
    .object({
      ...
      time: z.object({
        ...
        compactingBlocking: z.boolean().optional(),
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".voltcode", "plans")
  ...
  export const updatePart = fn(UpdatePartInput, async (input) => {
    ...
    // Fire-and-forget Bus.publish but catch errors to prevent unhandled rejections
    Bus.publish(MessageV2.Event.PartUpdated, {
      part,
      delta,
    }).catch((e) => {
      log.warn("Bus.publish error in updatePart", { partId: part.id, error: e })
    })
    return part
  })
