  ...
  export async function toModelMessage(input: WithParts[]): Promise<ModelMessage[]> {
    ...
    for (const msg of input) {
      ...
      if (msg.info.role === "assistant") {
        ...
        // Collect reasoning text from all reasoning parts for this message
        // This is needed for DeepSeek and similar models that require reasoning_content
        // to be set on assistant messages when continuing tool-calling conversations
        ...
        for (const part of msg.parts) {
          ...
          if (part.type === "tool") {
            if (part.state.status === "completed") {
              ...
              assistantMessage.parts.push({
                ...
                output: part.state.time.compacted ? "[Old tool result content cleared]" : safeOutput,
          ...
          if (part.type === "reasoning") {
            // Collect reasoning text for providerMetadata
            if (part.text) {
        ...
        // If we have reasoning text, attach it to the message's providerMetadata
        // This ensures it survives convertToModelMessages and can be used by the transform
        // to set reasoning_content for DeepSeek and similar models
        if (reasoningTexts.length > 0) {
          const combinedReasoning = reasoningTexts.join("")
          ;(assistantMessage as any).providerMetadata = {
            ...(assistantMessage as any).providerMetadata,
            openaiCompatible: {
