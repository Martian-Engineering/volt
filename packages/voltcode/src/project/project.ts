  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id =
          (await Filesystem.readText(path.join(dotgit, "voltcode")).catch(() => undefined)) ??
          (await Filesystem.readText(path.join(dotgit, "opencode")).catch(() => undefined))
        id = id?.trim()
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.VOLTCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "voltcode"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.VOLTCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
