...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        result = mergeConfigConcatArrays(
          result,
          await load(JSON.stringify(remoteConfig), `${key}/.well-known/voltcode`),
        )
    ...
    if (Flag.VOLTCODE_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(result, JSON.parse(Flag.VOLTCODE_CONFIG_CONTENT))
    ...
    if (Flag.VOLTCODE_CONFIG_DIR) {
      directories.push(Flag.VOLTCODE_CONFIG_DIR)
   ...
   * getPluginName("oh-my-voltcode@2.4.3") // "oh-my-voltcode"
   ...
   * 2. Local voltcode.json
   * 3. Global plugin/ directory
   * 4. Global voltcode.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-voltcode", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-voltcode@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Keybinds = z
    .object({
      ...
      tasktree_open: z.string().optional().default("ctrl+shift+t").describe("Open task tree view"),
  ...
  export const TUI = z.object({
    ...
    show_model_indicator: z
      .boolean()
      .optional()
      .default(false)
      .describe("Show model indicator in the UI (default: false)"),
  })
