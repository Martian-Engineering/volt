...
import type { Event, VoltcodeClient, SessionMessageResponse } from "@opencode-ai/sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: VoltcodeClient }) {
    return {
            ...
            switch (part.state.status) {
              ...
              case "completed": {
                ...
                if (part.tool === "todowrite") {
                  const parsedTodos = z.array(Todo.Info).safeParse(JSON.parse(safeOutput))
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // VoltCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const voltcodeProvider = providers.find((p) => p.id === "voltcode")
    if (voltcodeProvider) {
      if (voltcodeProvider.models["big-pickle"]) {
        return { providerID: "voltcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(voltcodeProvider.models))
      if (best) {
        return {
