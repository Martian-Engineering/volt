...
export namespace MCP {
  ...
  export async function tools() {
    ...
    for (const [clientName, client] of Object.entries(clientsSnapshot)) {
      ...
      const clientTimeout = entry?.timeout ?? defaultTimeout ?? DEFAULT_TIMEOUT

      const toolsResult = await withTimeout(client.listTools(), clientTimeout).catch(async (e) => {
        log.error("failed to get tools", { clientName, error: e.message, timeout: clientTimeout })
        const failedStatus = {
          status: "failed" as const,
      ...
      for (const mcpTool of toolsResult.tools) {
        ...
        result[sanitizedClientName + "_" + sanitizedToolName] = await convertMcpTool(mcpTool, client, clientTimeout)
      }
    }
    return result
  }
