    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in voltcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .voltcode/ subdirectory too)
  const candidates = [path.join(baseDir, "voltcode.json"), path.join(baseDir, "voltcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".voltcode", "voltcode.json"), path.join(baseDir, ".voltcode", "voltcode.jsonc"))
  }
  ...
  // Default to voltcode.json if none exist
  return candidates[0]
}
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "voltcode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
