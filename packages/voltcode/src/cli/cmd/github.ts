...
const AGENT_USERNAME = "voltcode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/voltcode.yml"
...
jobs:
  voltcode:
    ...
    steps:
      ...
      - name: Run voltcode
        uses: anomalyco/voltcode/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/voltcode,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("voltcode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `voltcode/${type}-${hex}-${timestamp}`
        }
        return `voltcode/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[voltcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the voltcode infrastructure after your response",
