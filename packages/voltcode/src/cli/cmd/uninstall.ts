...
export const UninstallCommand = {
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall VoltCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g voltcode-ai",
      pnpm: "pnpm uninstall -g voltcode-ai",
      bun: "bun remove -g voltcode-ai",
      yarn: "yarn global remove voltcode-ai",
      brew: "brew uninstall voltcode",
      choco: "choco uninstall voltcode",
      scoop: "scoop uninstall voltcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall voltcode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using VoltCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# voltcode") || content.includes(".voltcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".voltcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".voltcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".voltcode"))
    ) {
      continue
    }
