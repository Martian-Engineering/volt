...
import { createVoltcode, type ToolPart } from "@opencode-ai/sdk"
...
console.log("🚀 Starting voltcode server...")
const voltcode = await createVoltcode({
  port: 0,
})
console.log("✅ Voltcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await voltcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new voltcode session...")
    const { client, server } = voltcode
    ...
    console.log("✅ Created voltcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to voltcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Voltcode response:", JSON.stringify(result, null, 2))
