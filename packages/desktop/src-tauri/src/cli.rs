...
const CLI_INSTALL_DIR: &str = ".voltcode/bin";
const CLI_BINARY_NAME: &str = "voltcode";
...
const INSTALL_SCRIPT: &str = include_str!("../../../../packages/volt-gateway/install.sh");
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("voltcode-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("voltcode-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("VOLTCODE_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("VOLTCODE_CLIENT", "desktop")
