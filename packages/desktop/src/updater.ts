...
export const UPDATER_ENABLED = window.__VOLTCODE__?.updaterEnabled ?? false
...
export async function runUpdater({ alertOnFail }: { alertOnFail: boolean }) {
  ...
  if (!update) {
    if (alertOnFail)
      await message("You are already using the latest version of VoltCode", { title: "No Update Available" })
    return
  }
  ...
  const shouldUpdate = await ask(
    `Version ${update.version} of VoltCode has been downloaded, would you like to install it and relaunch?`,
