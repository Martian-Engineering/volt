...
export default function Layout(props: ParentProps) {
    ...
    async function pollUpdate() {
      ...
      if (updateAvailable && toastId === undefined) {
        toastId = showToast({
          ...
          description: `A new version of VoltCode (${version}) is now available to install.`,
          actions: [
            {
  ...
  const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
    ...
    const voltcode = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === voltcode ? "https://opencode.ai/favicon.svg" : props.project.icon?.override}
            ...
            <Show when={hoverReady()} fallback={<div class="text-12-regular text-text-weak">Loading messages…</div>}>
              <MessageNav
                ...
                onMessageSelect={(message) => {
                  if (!isActive()) {
                    sessionStorage.setItem("voltcode.pendingMessage", `${props.session.id}|${message.id}`)
      ...
      <div class="flex h-full w-full overflow-hidden">
        <div class="w-16 shrink-0 bg-background-base flex flex-col items-center overflow-hidden">
          <Show when={!sidebarProps.mobile}>
            <div class="shrink-0 w-full pt-2 pb-1 flex items-center justify-center">
              <TooltipKeybind placement="right" title="Toggle sidebar" keybind={command.keybind("sidebar.toggle")}>
                <IconButton
              ...
              <div class="shrink-0 px-2 py-3 border-t border-border-weak-base">
                <div class="rounded-md bg-background-base shadow-xs-border-base">
                  <div class="p-3 flex flex-col gap-2">
                    ...
                    <div class="text-text-base">VoltCode includes free models so you can start immediately.</div>
  ...
  return (
    <div class="relative bg-background-base flex-1 min-h-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text [&_main]:select-text">
      <Titlebar />
      <div class="flex-1 min-h-0 flex">
        ...
        >
          ...
          {/* ResizeHandle removed: expanded panel is now ChatSidebar in chat area */}
        </div>
        <div class="xl:hidden">
          <div
            classList={{
        ...
        <main
          classList={{
            "size-full overflow-hidden flex flex-col items-start contain-strict border-t border-border-weak-base": true,
