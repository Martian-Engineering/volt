...
type State = {
  ...
  lcm_file_loads: {
    [sessionID: string]: LcmFileLoadEvent[]
  }
}
...
function createGlobalSync() {
  ...
  const unsub = globalSDK.event.listen((e) => {
    ...
    switch (event.type) {
      ...
      case "session.lcm_file_loaded": {
        const sessionID = event.properties.sessionID
        const loadEvent: LcmFileLoadEvent = {
          sessionID,
          filePath: event.properties.filePath,
          fileId: event.properties.fileId,
          sizeBytes: event.properties.sizeBytes,
          tokenCount: event.properties.tokenCount,
          durationMs: event.properties.durationMs,
          timestamp: Date.now(),
        }
        const existingLoads = store.lcm_file_loads[sessionID] ?? []
        setStore("lcm_file_loads", sessionID, [...existingLoads, loadEvent])
        break
      }
    }
  })
      ...
      retry(() =>
        globalSDK.client.project.list().then(async (x) => {
          const projects = (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("voltcode-test"))
            .slice()
            .sort((a, b) => a.id.localeCompare(b.id))
