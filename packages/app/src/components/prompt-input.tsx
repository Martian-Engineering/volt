...
import { createVoltcodeClient, type Message, type Part } from "@opencode-ai/sdk/v2/client"
          ...
          <div class="flex items-center justify-start gap-0.5">
            <Switch>
              ...
              <Match when={store.mode === "normal"}>
                <Show when={local.agent.current()?.name === "plan"}>
                  <TooltipKeybind placement="top" title="Cycle agent" keybind={command.keybind("agent.cycle")}>
                    <Button variant="ghost" class="text-12-regular" onClick={() => local.agent.move(1)}>
                      <Icon name="checklist" size="small" class="text-icon-info-active" />
                      <span class="text-text-info-active">Plan Mode</span>
                    </Button>
                  </TooltipKeybind>
                </Show>
                <Show when={sync.data.config.tui?.show_model_indicator}>
                  <Show
