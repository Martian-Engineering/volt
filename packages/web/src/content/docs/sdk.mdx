---
title: SDK
description: Type-safe JS client for voltcode server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The voltcode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control voltcode programmatically.
...
Create an instance of voltcode:

```javascript
import { createVoltcode } from "@opencode-ai/sdk"

const { client } = await createVoltcode()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `voltcode.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${voltcode.server.url}`)
...
If you already have a running instance of voltcode, you can create a client instance to connect to it:
...
const textResults = await client.find.text({
  query: { pattern: "function.*voltcode" },
})
