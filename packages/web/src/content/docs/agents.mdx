...
There are two types of agents in VoltCode; primary agents and subagents.
...
VoltCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
VoltCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

VoltCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `voltcode.json` config file:
...
- Global: `~/.config/voltcode/agents/`
- Per-project: `.voltcode/agents/`

```markdown title="~/.config/voltcode/agents/review.md"
---
...
If no temperature is specified, VoltCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global VoltCode config and the project specific config.
...
The model ID in your VoltCode config uses the format `provider/model-id`. For example, if you're using [VoltCode Zen](/docs/zen), you would use `voltcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="voltcode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="voltcode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `voltcode models` to see a list of the available models.
:::
...
```markdown title="~/.config/voltcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/voltcode/agents/security-auditor.md"
---
