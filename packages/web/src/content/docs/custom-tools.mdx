---
title: Custom Tools
description: Create tools the LLM can call in voltcode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside voltcode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.voltcode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/voltcode/tools/`.
...
```ts title=".voltcode/tools/database.ts" {1}
...
```ts title=".voltcode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".voltcode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".voltcode/tools/add.py"
import sys
...
```ts title=".voltcode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .voltcode/tools/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
