---
title: Server
description: Interact with voltcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `voltcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an voltcode client can use.
...
```bash
voltcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
voltcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `VOLTCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `voltcode`, or set `VOLTCODE_SERVER_USERNAME` to override it. This applies to both `voltcode serve` and `voltcode web`.

```bash
VOLTCODE_SERVER_PASSWORD=your-password voltcode serve
```
...
When you run `voltcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the voltcode server to interact with voltcode programmatically.
:::

This architecture lets voltcode support multiple clients and allows you to interact with voltcode programmatically.

You can run `voltcode serve` to start a standalone server. If you have the
voltcode TUI running, `voltcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the VoltCode [IDE](/docs/ide) plugins.
...
The voltcode server exposes the following APIs.
