...
The `volt serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that a VoltCode client can use.
...
```bash
volt serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
volt serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `VOLTCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `voltcode`, or set `VOLTCODE_SERVER_USERNAME` to override it. This applies to both `volt serve` and `volt web`.

```bash
VOLTCODE_SERVER_PASSWORD=your-password volt serve
```
...
When you run `volt` it starts a TUI and a server. Where the TUI is the
...
You can run `volt serve` to start a standalone server. If you have the
VoltCode TUI running, `volt serve` will start a new server.
