...
You can add external tools to VoltCode using the _Model Context Protocol_, or MCP. VoltCode supports both local and remote servers.
...
You can define MCP servers in your [VoltCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="voltcode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/voltcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
VoltCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, VoltCode will:
...
If the server requires authentication, VoltCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `voltcode mcp auth <server-name>`.
...
```bash
voltcode mcp auth my-oauth-server
```
...
```bash
voltcode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, VoltCode will store the tokens securely in `~/.local/share/voltcode/mcp-auth.json`.
...
voltcode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in VoltCode, alongside built-in tools. So you can manage them through the VoltCode config like any other tool.
...
```json title="voltcode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
voltcode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect VoltCode to your Sentry account.
