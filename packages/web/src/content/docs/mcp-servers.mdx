...
If the server requires authentication, VoltCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `volt mcp auth <server-name>`.
...
```bash
volt mcp logout my-oauth-server
```
...
volt mcp debug my-oauth-server
```
