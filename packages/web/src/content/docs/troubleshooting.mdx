...
To debug any issues with VoltCode, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/voltcode/log/`
- **Windows**: `%USERPROFILE%\.local\share\voltcode\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `voltcode --log-level DEBUG`.

---

### Storage

voltcode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/voltcode/`
- **Windows**: `%USERPROFILE%\.local\share\voltcode`
...
If you're experiencing issues with VoltCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/voltcode/issues**](https://github.com/anomalyco/opencode/issues)
...
### VoltCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `voltcode upgrade`
...
To figure out what models you have access to, run `voltcode models`
...
If you encounter API call errors, this may be due to outdated provider packages. voltcode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/voltcode
   ```

2. Restart voltcode to reinstall the latest provider packages

This will force voltcode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
voltcode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
