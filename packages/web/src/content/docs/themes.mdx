...
With VoltCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, VoltCode uses our own `voltcode` theme.
...
VoltCode comes with several built-in themes.
...
- Want VoltCode to match their terminal's appearance
...
VoltCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/voltcode/themes/*.json` or `$XDG_CONFIG_HOME/voltcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.voltcode/themes/*.json`
4. **Current working directory** - Defined in `./.voltcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/voltcode/themes
vim ~/.config/voltcode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .voltcode/themes
vim .voltcode/themes/my-theme.json
```
