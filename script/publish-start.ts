...
if (!Script.preview) {
  const previous = await getLatestRelease().catch(() => null)
  if (previous) {
...
console.log("\n=== voltcode ===\n")
await import(`../packages/voltcode/script/publish.ts`)
